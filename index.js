...
function createArray(...elements) {
  if (elements.length > 0) {
    let array = Array.of(...elements);
    return array;
  } else {
    console.log("Помилка: необхідно передати хоча б один елемент");
    return [];
  }
}
 ...
 */
function copyAndSwapElements(arr, target, start = 0, end = arr.length) {
  if (
    target < 0 ||
    target >= arr.length ||
    start < 0 ||
...
function customFill(arr, value, start = 0, end = arr.length) {
  ...
  let slice = copy.slice(start, end);

  let filled = slice.fill(value);

  copy.splice(start, end - start, ...filled);
...
function customUnshift(arr, ...elements) {
  if (!Array.isArray(arr)) {
    console.log("Перший параметр має бути масивом");
  }
